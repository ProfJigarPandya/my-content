...
{
	int total_chain_nodes=-1, existing_chain_nodes=1;
	...
	{
		int wstatus;
		pid_t cpid[total_chain_nodes],w;
		for(int t=2;t<=total_chain_nodes;t++)
		{
			cpid[t]=fork();
			if(cpid[t]==-1)
			{
			...
			{
				printf("\n CHILD: (%d\t%d\t%d\t%d)\n",getpid(),getppid(),existing_chain_nodes,total_chain_nodes);
				fflush(stdout);
				exit(EXIT_SUCCESS);
			}
		...
		for(int t=2;t<=total_chain_nodes;t++)
		{
				...
				do {
                   			w = waitpid(cpid[t], &wstatus, WUNTRACED | WCONTINUED);
                   			if (w == -1) {
                       				perror("waitpid");
                       				exit(EXIT_FAILURE);
                   			}
